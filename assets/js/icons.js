/**
 * icons.js – Lucide-style SVG icon constants (global ICON object)
 * Include BEFORE layout.js and page scripts.
 */
const ICON = {
...
function renderIcons() {
    ...
    // Clean up any literal ${ICON...} strings left in HTML
    traverseAndReplace(document.body);
}

function traverseAndReplace(node) {
    if (node.nodeType === 3) {
        if (node.nodeValue.includes('${ICON.')) {
            const span = document.createElement('span');
            span.innerHTML = node.nodeValue.replace(/\$\{ICON\.([a-zA-Z]+)\}/g, (m, k) => ICON[k] || '');
